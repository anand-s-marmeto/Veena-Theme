class PincodeChecker extends HTMLElement {
  constructor() {
    ...
    this.data =
      JSON.parse(localStorage.getItem("pincodeSheetData")) ||
  ...
  async checkPincodeAvailability() {
    ...
    try {
      ...
      if (result) {
        ...
        const EstDeliveryNode = this.querySelector(
          "[data-pincode-estdelivery]"
        );
        ...
        if (result[deliverableIndex] === "TRUE") {
          ...
          this.estDeliveryNode.textContent =
            EstDeliveryNode.textContent.replace(
              "[est_Days]",
              result[estDaysIndex]
            );
  ...
  fetchSheetsData() {
    ...
    const SPREADSHEET_ID = "1xJWww2FaxyuD-Wn7nIgd4G6EvKNi2IWBnFGfGyelUeo";
    const RANGE = "Sheet1";
  ...
  disableAddButton(disable) {
    document
      ...
      ?.toggleAddButton(disable, "Add pincode");
    document
      .querySelector("variant-selects")
      ?.toggleAddButton(disable, "Add pincode");

    document
      .querySelectorAll('form[action="/cart/add"] button')
      .forEach((button) => {
        if (disable) button.setAttribute("title", "Add a pincode to continue!");
